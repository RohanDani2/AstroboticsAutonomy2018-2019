function LidarMap = createLidarMap(data)%create occupancy map out of lidar scan. PASS ADJUSTED DATA HERE 
    ...
    compressedLidarMap = zeros(360,570); %%change size to 36 x 57 for full compression 
    q = 1;
    for i = 1:10:3590 %y change to 1:100:3500 for full compression 
        r = 1; 
        for j = 1:10:5690%x change to 1:100:5600 for full compression 
            for k = i:(i+10) %change to i + 100
                for l = j:(j+10) % change to i + 100 
                    if (fullLidarMap(k,l) == 1)
                        compressedLidarMap(q,r) = 1;
                    end
                end
            end
            r = r+1;
...
figure(3)
title ('Compressed Lidar Map by 10')
hold on
grid on 
for j = 1:length(compressedLidarMap(1,:))
    for k = 1:length(compressedLidarMap(:,1))
        if (compressedLidarMap(k,j) == 1)
            plot(j,k,'b.')
        end
