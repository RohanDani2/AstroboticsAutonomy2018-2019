function path  = Astar_Ryan(startPos,endPt,allObstacles)
Connecting_Distance = 1;
GoalRegister=int8(zeros(575,369));
GoalRegister(endPt(1),endPt(2))=1;
CurrentY = 0;
CurrentX = 0; 
MAP = zeros(575 , 369); % creates an array of zeros for all of the points in the map. it's a 1 for obstacles and 0 for everything else
for i = 1:(length(allObstacles)-1)%puts obstacles and wall in map 
    MAP(allObstacles(i,1),allObstacles(i,2)) = 1;
end
...
FScore(startPos(1),startPos(2))=Hn(startPos(1),startPos(2));         
OpenMAT(startPos(1),startPos(2))=1;   
...
k=2;
path = zeros(500,2);
if RECONSTRUCTPATH %come up with way for path to be initialized as a n x 2 matrix OR figure out how to just put in the coordinates in propery (most likely both)
    %path(1,:)=[CurrentY CurrentX];
    path(1,1) = CurrentY;
    path(1,2) = CurrentX; 
    while RECONSTRUCTPATH
        ...
        %path(k,:)=[CurrentY CurrentX];
        path(k,1) = CurrentY; 
        path(k,2) = CurrentX; 
        k=k+1;
        if (((CurrentX== startPos(2))) &&(CurrentY==startPos(1)))
            break
        end
    end
end
for i = 1:length(path) % replaces the zeros in path with the start point 
